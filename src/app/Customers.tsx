    ...
    <div className="p-5 space-y-4">
      <div className="flex items-center justify-between">
        ...
        <Dialog>
          <DialogTrigger
            render={
              <Button className="bg-[#38bdf8] hover:bg-[#38bdf8]/90 text-[#020617] gap-2 h-9 text-xs font-bold px-4 flex items-center justify-center rounded-lg">
      ...
      <div className="flex items-center gap-3 bg-[#0f172a] p-3 rounded-xl border border-[#1e293b]">
        <div className="relative flex-1">
          ...
          <Input 
            ...
            className="bg-[#1e293b] border-[#334155] pr-10 h-9 text-xs focus-visible:ring-[#38bdf8]/50"
            value={searchTerm}
          ...
          <table className="w-full text-right text-[12px]">
            <thead>
              <tr className="border-b border-[#1e293b] bg-[#1e293b]/20">
                <th className="px-5 py-3 font-medium text-[#94a3b8]">نام کامل</th>
                <th className="px-5 py-3 font-medium text-[#94a3b8]">شرکت</th>
                <th className="px-5 py-3 font-medium text-[#94a3b8]">اطلاعات تماس</th>
                <th className="px-5 py-3 font-medium text-[#94a3b8]">تعداد محموله</th>
                <th className="px-5 py-3 font-medium text-[#94a3b8]">تاریخ ایجاد</th>
                <th className="px-5 py-3 font-medium text-[#94a3b8]">عملیات</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-[#1e293b]">
              {filteredCustomers.map((customer) => (
                <tr key={customer.id} className="hover:bg-[#1e293b]/30 transition-colors group">
                  ...
                  <td className="px-5 py-3 text-slate-300 font-medium">{(customer as any).company}</td>
                  <td className="px-5 py-3">
                  ...
                  <td className="px-5 py-3 text-slate-500">{customer.createdAt}</td>
                  <td className="px-5 py-3">
                    <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      <Button variant="ghost" size="icon" className="h-7 w-7 text-slate-500 hover:text-white hover:bg-[#1e293b]" onClick={() => window.open('/track', '_blank')}>
                        <ExternalLink className="w-3.5 h-3.5" />
                      </Button>
                      <DropdownMenu>
                        <DropdownMenuTrigger
                          render={
                            <Button variant="ghost" size="icon" className="h-7 w-7 text-slate-500 hover:text-white hover:bg-[#1e293b]">
                              <MoreVertical className="w-3.5 h-3.5" />
                            </Button>
                          }
                        />
                        <DropdownMenuContent className="bg-[#0f172a] border-[#1e293b] text-[#f8fafc] text-right" dir="rtl">
                          <DropdownMenuItem 
                            className="text-xs cursor-pointer hover:bg-[#1e293b] flex items-center justify-between gap-2 text-[#38bdf8] font-bold"
                            onClick={() => {
                          ...
                          >
                            اشتراک‌گذاری پنل رهگیری
                            <Share2 className="w-3.5 h-3.5" />
                          </DropdownMenuItem>
                          <DropdownMenuItem className="text-xs cursor-pointer hover:bg-[#1e293b]">مشاهده تاریخچه</DropdownMenuItem>
                          <DropdownMenuItem className="text-xs cursor-pointer hover:bg-[#1e293b] text-red-400">حذف مشتری</DropdownMenuItem>
                        </DropdownMenuContent>
                      </DropdownMenu>
                    </div>
                  </td>
